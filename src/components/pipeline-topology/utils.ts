...
import { CheckTaskErrorMessage } from '../pipelines-details/types';
...
export const getLastRegularTasks = (
  regularTasks: PipelineMixedNodeModel[],
): string[] => {
...
export const getTopologyNodesEdges = (
  ...
  const taskList: PipelineTask[] = _.flatten(
    getPipelineTasks(pipeline, pipelineRun, taskRuns),
  );
  const taskNodes: PipelineMixedNodeModel[] = tasksToNodes(
    taskList,
    pipeline,
    pipelineRun,
  );
...
export const taskHasWhenExpression = (task: PipelineTask): boolean =>
  task?.when?.length > 0;

export const nodesHasWhenExpression = (
  nodes: PipelineMixedNodeModel[],
): boolean => nodes.some((n) => taskHasWhenExpression(n.data.task));

export const hasWhenExpression = (pipeline: PipelineKind): boolean => {
  return [
    ...(pipeline?.spec?.tasks || []),
    ...(pipeline?.spec?.finally || []),
  ].some(taskHasWhenExpression);
};
...
export const getWhenExpressionDiamondState = (
  ...
  switch (status) {
    ...
    case ComputedStatus.Failed:
      tooltipContent = t('When expression was met');
      break;
    case ComputedStatus.Skipped:
      tooltipContent = t('When expression was not met');
      break;
    default:
