...
class DropDownRowWithTranslation extends React.PureComponent {
  render() {
      ...
      prefix = (
        <a
          ...
          className={classNames(
            ...
            { 'pf-v6-m-favorited': isBookmarked },
          )}
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          aria-label={
            isBookmarked
              ? t(
                  'plugin__pipelines-console-plugin~Remove bookmark {{content}}',
                  {
                    content: contentString,
                  },
                )
              : t('plugin__pipelines-console-plugin~Add bookmark {{content}}', {
                  content: contentString,
                })
          }
        >
      ...
      suffix = (
        <a
          ...
          aria-label={
            isFavorite
              ? t(
                  'plugin__pipelines-console-plugin~Remove favorite {{content}}',
                  {
                    content: contentString,
                  },
                )
              : t('plugin__pipelines-console-plugin~Add favorite {{content}}', {
                  content: contentString,
                })
          }
        >
    ...
    return (
      <li
        role="option"
        className={classNames('pf-v6-c-menu__list-item', className)}
        key={itemKey}
      >
        <a
          href="#"
...
class Dropdown_ extends DropdownMixin {
  constructor(props) {
    ...
    this.changeTextFilter = (e) =>
      this.applyTextFilter_(e.target.value, this.props.items);
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      _.isEqual(nextProps.items, props.items) &&
  ...
  applyTextFilter_(autocompleteText, items) {
    ...
    if (autocompleteFilter && !_.isEmpty(autocompleteText)) {
      items = _.pickBy(items, (item, key) =>
        autocompleteFilter(autocompleteText, item, key),
      );
    }
      ...
      return (
        <>
          {actionItems.map((ai) => (
            <DropDownRow
              className={classNames({
                active: ai.actionKey === selectedKey && !noSelection,
              })}
  ...
  render() {
      ...
      return (
        ...
        >
          <div
            className={classNames(
              'pf-v6-c-dropdown',
              { 'pf-v6-m-expanded': this.state.active },
              dropDownClassName,
            )}
          >
            <button
            ...
            >
              <div
                className={classNames(
                  'pf-v6-c-dropdown__content-wrap',
                  dropDownContentClassName,
                )}
              >
              ...
              <div className="pf-v6-c-menu pf-m-scrollable co-namespace-dropdown__menu">
                ...
                >
                  ...
                  >
                    ...
                    {bookMarkRows}
                    {_.size(bookMarkRows) && _.size(rows) ? (
                      <Divider component="li" />
                    ) : null}
                    {rows}
                  </ul>
    ...
    return (
      ...
      >
        <div
          className={classNames(
            { 'pf-v6-c-dropdown': true, 'pf-v6-m-expanded': this.state.active },
            dropDownClassName,
          )}
        >
          <button
...
export const ContainerDropdown = (props) => {
  ...
  const getHeaders = (container, initContainer) => {
    return initContainer
      ? {
          [container.name]: t('plugin__pipelines-console-plugin~Containers'),
          [initContainer.name]: t(
            'plugin__pipelines-console-plugin~Init containers',
          ),
        }
      : {};
  };
  ...
  const dropdownItems = _.mapValues(
    _.merge(containers, initContainers),
    containerLabel,
  );
  const title =
    _.get(dropdownItems, currentKey) || containerLabel(firstContainer);
  return (
    <Dropdown
...
ContainerDropdown.propTypes = {
  containers: PropTypes.oneOfType([PropTypes.object, PropTypes.array])
    .isRequired,
